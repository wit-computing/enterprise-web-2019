...
The component renders slightly different depending on whether the news item article has a hyperlink or not. Add two story to `stories/hackerApp.js`:
...
The news item and news form stories are under the same group name (Hacker App). Storybook allows multiple invocations of its *storiesOf()* in the same file - it's a matter of preference.
